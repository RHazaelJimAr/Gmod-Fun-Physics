SWEP.PrintName = "Bath Bomber" -- This will be shown in the spawn menu, and in the weapon selection menu
SWEP.Author = "Randy" -- These two options will be shown when you have the weapon highlighted in the weapon selection menu
SWEP.Instructions = "Left mouse to spawn a Physics Bathtub, click again to relocate the Bathtub"
...
function SWEP:ThrowBathtub(isCleanup)
    ...
    if not isCleanup then
        ...
        Spawn_SENT(owner, "testent", trace)
    else
        for k, v in ipairs(ents.FindByClass("testent")) do
            if v and v:IsValid() then v:Remove() end
        end
