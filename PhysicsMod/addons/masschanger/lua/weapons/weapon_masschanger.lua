SWEP.PrintName = "Mass Changer" -- This will be shown in the spawn menu, and in the weapon selection menu
SWEP.Author = "Randy" -- These two options will be shown when you have the weapon highlighted in the weapon selection menu
SWEP.Instructions = "Left mouse to increase object mass, right mouse to decrease it"
...
function SWEP:MassAddition(mass)
    ...
    if (IsValid(traceRes.Entity) && IsValid(traceRes.Entity:GetPhysicsObject())) then
		...
		phys:SetMass(phys:GetMass() + mass)
	end
end